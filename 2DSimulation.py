...
def twoD(beta,gamma,N,I0,T,contacts,plotter):
    prob = beta/contacts
    ratio = (contacts+1)/9
    ...
    model_df,frames = model(df,data_h,T,N_root,prob,gamma)
    if plotter == 1:
...
def infect(df,N_root,prob,gamma):
    ...
    for n in range(0,len(i_list)):
        for i in [-1,0,1]:
            for j in [-1,0,1]:
                ...
                if (trail_x != -1 and trail_y != -1):
                    if (trail_x != N_root and trail_y != N_root):
...
def plot(df):
    fig1= go.Figure(data=go.Heatmap(df_to_plotly(df), colorscale=["black", "blue","red", "green"],zmin=0, zmax=3))
    fig1.update_layout(
    font=dict(
        family="Arial Black",
...
def model(df,data_h,T,N_root,prob,gamma):
    s = np.empty(T)
    i = np.empty(T)
    r = np.empty(T)
    t = np.empty(T)
    ...
    t[0] = 0
    for dt in range(1,T):
        df = infect(df,N_root,prob,gamma)
...
def animation(df,data_h,T):
    fig = go.Figure(data=go.Heatmap( df_to_plotly(df), colorscale=["black", "blue","red", "green"], zmin=0, zmax=3),
                    frames=[go.Frame(data = go.Heatmap(data_h[i], colorscale=["black", "blue","red", "green"]))for i in range(T)])
    fig.update_layout(
    font=dict(
        family="Arial Black",
        size=26,  # Set the font size here
